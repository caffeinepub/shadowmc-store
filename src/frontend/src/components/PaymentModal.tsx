...
export default function PaymentModal({ open, onOpenChange, total }: Props) {
  ...
  const [screenshotPreviewUrl, setScreenshotPreviewUrl] = useState<
    string | null
  >(null);
  // Controls the "Go to History" info popup shown after order completion
  ...
  const handleSubmit = () => {
    ...
    if (valid) {
      ...
      setStep(5);
      // Show the "Go to History" info popup automatically
  ...
  return (
    <>
      ...
      {/* "Go to History" info popup — shown after order is submitted */}
      <AnimatePresence>
        {showHistoryPopup && (
          <motion.div
            key="history-popup"
